...
-- -----------------------------------------------------------------------
-- Regression: heap filter expression containing a scalar SubPlan
-- (PARAM_EXEC). Previously segfaulted inside HeapFieldFilter because
-- CreateStandaloneExprContext lacks ecxt_param_exec_vals. Custom scan
-- must reject such predicates and let Postgres apply them as a Filter
-- above an Index Scan. See qual_inspect.rs try_pushdown().
-- -----------------------------------------------------------------------

-- Scalar SubPlan compared to non-indexed boolean heap column
SELECT id, description, in_stock
FROM products_hp
WHERE description @@@ 'laptop'
  AND in_stock = (SELECT true)
ORDER BY id;

-- Two scalar SubPlans combined with BM25
SELECT COUNT(*)
FROM products_hp
WHERE description @@@ 'laptop'
  AND in_stock = (SELECT true)
